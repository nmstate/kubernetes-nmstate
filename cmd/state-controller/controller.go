...
func NewController(
	kubeclientset kubernetes.Interface,
	nmstateclientset clientset.Interface,
	stateInformer informers.NodeNetworkStateInformer) *Controller {
...
func (c *Controller) syncHandler(key string) error {
	...
	if nmstatectl.IsStateApplicable(nil, state) {
		err = nmstatectl.HandleResource(state, c.nmstateclientset.NmstateV1())
		if err != nil {
			return err
		}
		c.recorder.Event(state, corev1.EventTypeNormal, SuccessSynced, MessageResourceSynced)
	} else {
		fmt.Printf("NodeNetworkState %s' not applicable to node\n", name)
	}
