...
Manifests are generated in `build/_output/manifests/` by rendering the Helm chart in `charts/kubernetes-nmstate/`; the rendered operator manifests live under `build/_output/manifests/kubernetes-nmstate/templates/`. The chart templates get populated with correct image references during `cluster-sync`, while `make manifests` remains available for manifest-based deployment flows.
