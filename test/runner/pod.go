...
import (
	...
	kubectl "github.com/nmstate/kubernetes-nmstate/test/kubectl"
)

func nmstatePods() ([]string, error) {
	output, err := kubectl.Kubectl("get", "pod", "-n", framework.Global.Namespace, "--no-headers=true", "-o", "custom-columns=:metadata.name")
...
func RunAtPods(arguments ...string) {
	...
	for _, nmstatePod := range nmstatePods {
		...
		_, err := kubectl.Kubectl(execArguments...)
		ExpectWithOffset(1, err).ToNot(HaveOccurred())
	}
}
