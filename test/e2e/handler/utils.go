...
func setDesiredStateWithPolicyAndNodeSelector(name string, desiredState nmstatev1beta1.State, nodeSelector map[string]string) {
...
func setDesiredStateWithPolicy(name string, desiredState nmstatev1beta1.State) {
...
func updateDesiredState(desiredState nmstatev1beta1.State) {
	setDesiredStateWithPolicy(TestPolicy, desiredState)
}

func updateDesiredStateAndWait(desiredState nmstatev1beta1.State) {
	updateDesiredState(desiredState)
	waitForAvailableTestPolicy()
}

func updateDesiredStateAtNode(node string, desiredState nmstatev1beta1.State) {
	nodeSelector := map[string]string{"kubernetes.io/hostname": node}
	setDesiredStateWithPolicyAndNodeSelector(TestPolicy, desiredState, nodeSelector)
}

func updateDesiredStateAtNodeAndWait(node string, desiredState nmstatev1beta1.State) {
...
func resetDesiredStateForNodes() {
	...
	updateDesiredState(nmstatev1beta1.NewState(fmt.Sprintf(`interfaces:
  - name: %s
    type: ethernet
    state: up
  - name: %s
...
func deletePolicy(name string) {
	...
	for _, node := range nodes {
		enactmentKey := nmstatev1beta1.EnactmentKey(node, name)
...
func interfaces(state nmstatev1beta1.State) []interface{} {
...
func currentState(node string, currentStateYaml *nmstatev1beta1.State) AsyncAssertion {
	key := types.NamespacedName{Namespace: framework.Global.Namespace, Name: node}
	return Eventually(func() nmstatev1beta1.RawState {
...
func interfacesNameForNode(node string) []string {
	var currentStateYaml nmstatev1beta1.State
