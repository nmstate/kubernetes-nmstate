...
cluster-clean: $(KUBECTL)
	...
	$(KUBECTL) delete --ignore-not-found -f deploy/crds/nmstate_v1_nodenetworkstate_cr.yaml
	$(KUBECTL) delete --ignore-not-found nodenetworkstate --all
...
cluster-sync-handler: $(local_handler_manifest) $(CLI) $(KUBECTL)
