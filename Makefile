...
# Currently limited to read-only NNS tests.
# Tests that apply NNCPs (OVS, VLAN, Bridge, Bond, StaticAddress, etc.) need
# validation that nmstatectl apply -k works for each type on kind before
# being added here.
KERNEL_MODE_FEATURES ?= (Nodes && !NodeSSH) || NNSDependencies
