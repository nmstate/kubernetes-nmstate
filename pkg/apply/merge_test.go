  ...
  annotations:
    ...
    c: upd`)
		...
		It("should merge annotations", func() {
			// Create a fresh copy to avoid shared state issues
			updCopy := unstructuredFromYaml(`
apiVersion: v1
kind: Namespace
metadata:
  name: ns1
  ...
  annotations:
    ...
    c: upd`)
			...
			err := apply.MergeObjectForUpdate(cur, updCopy)
			Expect(err).NotTo(HaveOccurred())

			Expect(updCopy.GetAnnotations()).To(Equal(map[string]string{
				"a": "upd",
				"b": "cur",
				"c": "upd",
			}))
...
metadata:
  ...
  annotations:
    ...
    c: upd`)
			...
			Expect(updCopy.GetLabels()).To(Equal(map[string]string{
				"a": "upd",
				"b": "cur",
				"c": "upd",
			}))
...
spec:
  clusterIP: upd`)
			...
			ip, _, err := unstructured.NestedString(updCopy.Object, "spec", "clusterIP")
			Expect(err).NotTo(HaveOccurred())
...
metadata:
  ...
  annotations:
    b: upd`)
			...
			s, ok, err := unstructured.NestedSlice(updCopy.Object, "secrets")
			Expect(err).NotTo(HaveOccurred())
			Expect(ok).To(BeTrue())
...
metadata:
  name: d1`)
			...
			Expect(updCopy.GetLabels()).To(BeEmpty())
		})
	})
