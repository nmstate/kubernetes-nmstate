...
function format() {
    git ls-files | grep -v "^vendor/" | grep -v "^pkg/names/vendor/" | grep -v "^api/vendor/" | grep -v "logo/" | xargs sed --follow-symlinks -i 's/[[:space:]]*$//'
}

function check() {
    invalid_files=$(git ls-files | grep -v "^vendor/" | grep -v "^pkg/names/vendor/" | grep -v "^api/vendor/" | xargs egrep -Hn " +$" || true)
